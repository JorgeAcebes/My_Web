(function() {
    ...
    const render = () => {
        const basePath = '/My_Web';
        const targetPath = basePath + '/Renfe';
        if (window.location.pathname !== targetPath) {
            window.history.pushState({ view: 'renfe' }, '', targetPath);
        }
            ...
            <div id="renfeModal" class="modal">
                <div style="max-width:450px; margin:2rem auto; border:1px solid var(--border); padding:2rem; background:var(--bg); box-shadow: 20px 20px 0px rgba(0,0,0,0.05);">
                    ...
                    <div style="display:flex; flex-direction:column; gap:8px; margin-bottom:2rem; padding:1.5rem; border:1px dashed #ccc;">
                        <input type="text" id="newRenfeName" placeholder="Nombre trayecto" style="width:100%; padding:10px; border:1px solid var(--border); font-family:inherit; text-transform:uppercase; font-size:0.75rem;">
                        <input type="text" id="newRenfeUrl" placeholder="URL Renfe" style="width:100%; padding:10px; border:1px solid var(--border); font-family:inherit; font-size:0.75rem;">
    ...
    const renderModalList = () => {
        ...
        Object.entries(state).forEach(([id, route]) => {
            ...
            div.innerHTML = `
                <input type="text" value="${route.name}" style="flex:1; padding:8px; border:1px solid #eee; font-family:inherit; font-size:0.75rem; text-transform:uppercase;" id="input-${id}">
